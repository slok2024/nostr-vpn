...
- New unit tests for the Windows `route print -4 0.0.0.0` parser (3 tests covering the happy path, On-link skip, and missing-default cases), runnable on all platforms via `cargo test -p nostr-vpn-cli --features embedded-fips`. 110 tests pass.

### Notes / not yet done

- **iOS / Android** still don't have WG upstream. The userspace runtime now exposes `WgUpstreamRuntime::start_with_channels` so the Rust side is ready to plug in, but the OS-side glue (NEPacketTunnelProvider on iOS / VpnService on Android) needs to declare the routing through `NEPacketTunnelNetworkSettings` / `VpnService.Builder` and protect the boringtun UDP socket from the VPN tunnel itself. That's out of scope for this round.
